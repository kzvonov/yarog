...
module Dw
  class BardSetup
    ...
    # Creates and saves a Bard Hero to the database
    def create_bard(name: nil, code: nil, options: {})
      reset_character_data
      ...
      select_bardic_lore(options[:bardic_lore])
    ...
    def available_lore_specializations
      bardic_lore = @class_data["moves"]["level_1"].find { |m| m["id"] == "bardic_lore" }
      bardic_lore&.dig("specialization", "choose_one") || []
    end
    ...
    def format_move_description(move)
      ...
      if move["questions"]
        parts << "Вопросы: #{move['questions'].join('; ')}"
      end
    ...
    def format_equipment
      ...
      if @character_data[:instrument]
        equipment_lines << "Инструмент: #{@character_data[:instrument]} (0 вес)"
      end

      # Add weapons
      weapons = @character_data[:gear].select do |item|
        item.is_a?(Hash) && (item["item"]&.include?("рапира") || item["item"]&.include?("меч") ||
                             item["item"]&.include?("лук") || item["tags"])
      end
      ...
      if weapons.any?
        ...
        weapons.each do |weapon|
          ...
          ammo = weapon["ammo"] ? " x#{weapon['ammo']}" : ""
          equipment_lines << "- #{name}#{tags}#{ammo}#{weight}"
        end
      end
      ...
      @character_data[:gear].each do |item|
        ...
        formatted_item = "#{name}#{uses}#{armor_val}#{weight}"

        # Categorize items
        if item["armor"] || name.include?("броня")
          armor_items << formatted_item
        elsif name.include?("Провизия") || name.include?("Бинты") || name.include?("одежда")
      ...
      equipment_lines << "\nЗапасы и снаряжение:" if consumables.any?
      equipment_lines.concat(consumables.map { |item| "- #{item}" })

      equipment_lines << "\nПрочее:" if other_items.any?
    ...
    def calculate_total_weight
      ...
      # Add gear weight (instrument is 0 for bard)
      @character_data[:gear].each do |item|
    ...
    def format_notes
      ...
      # Add bardic lore specialization
      if @character_data[:bardic_lore]
        notes << "\nБардовские знания: #{@character_data[:bardic_lore]}"
      end
    ...
    def load_class_data
      file_path = File.join(__dir__, "classes", "ru_bard.json")
    ...
    def select_bardic_lore(lore_choice = nil)
      specializations = available_lore_specializations
      return if specializations.empty?

      # If no choice provided, pick randomly
      lore_idx = lore_choice || rand(specializations.length)
      @character_data[:bardic_lore] = specializations[lore_idx]
    end
    ...
    def select_gear(choices = {})
      ...
      # Instrument choice (choose one)
      instrument_choice = choices[:instrument] || 0
      instruments = gear_data["choose_instrument_one"]["options"]
      @character_data[:instrument] = instruments[instrument_choice] || instruments.first

      # Clothing choice (armor or fancy clothes)
      clothing_choice = choices[:clothing] || 0
      selected_clothing = gear_data["choose_clothing_one"][clothing_choice]
      if selected_clothing
        @character_data[:gear] << selected_clothing
        @character_data[:armor] += selected_clothing["armor"] if selected_clothing["armor"]
      end
      ...
      @character_data[:gear] << selected_additional if selected_additional
    end
